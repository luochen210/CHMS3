...
namespace XlsReport
{
    ...
    {
        private static int _PrintMode = 1;  //0表示Preview,1表示Print

        public static int PrintMode
        {
            get { return PrintDGV._PrintMode; }
            set { PrintDGV._PrintMode = value; }
        }
        ...
        private static bool LandScape = false;                 //是否横向打印
        ...
        //***************
        //描述信息
        private static ArrayList _description = new ArrayList();
        /// <summary>
        /// 描述信息
        /// </summary>
        ...
        /// 打印DataGridView
        /// </summary>
        /// <param name="dgv1">DataGridView,</param>
        public static void Print_DataGridView(DataGridView dgv1)
        {
	        ...
	        {	
                ...
                if (_PrintMode == 0)
                {
                    FrmPrintPreview dlg = new FrmPrintPreview();
                    if (dlg.ShowDialog() != DialogResult.OK) return;
                    SelectedColumns = AvailableColumns;
                    PrintAllRows = true;
                    FitToPageWidth = true;
                    LandScape = dlg.LandPrint;
                    RowsPerPage = 0;
                    ...
                    else
                    {
                        if (NewPage)
                        {
                            ...
                            if (_title != null)
                            {
                                ...
                                e.Graphics.DrawString(_title, drawFont, drawBrush, drawRect, drawFormat);
                            ...
                            if (count > 0)
                            {
                                ...
                                for (int index = 0; index < count; index++)
                                {
                                    ...
                                    if (flag % 2 == 1)
                                    {
                                        ...
                                        YPoint += 25.0F;
                                        string[,] descriptemparr = (string[,])_description[index];
                                        string caption = descriptemparr[0, 0];
                                        string content = descriptemparr[0, 1];
                                        ...
                                        PointF drawPoint = new PointF(XPoint, YPoint);

                                        // Set format of string.
                                        StringFormat drawFormat = new StringFormat();
                                        drawFormat.FormatFlags = StringFormatFlags.DisplayFormatControl;

                                        e.Graphics.DrawString(caption, drawFont, drawBrush, drawPoint, drawFormat);
                                        drawPoint.X = XPoint + 100.0F;
                                        drawPoint.Y = YPoint;
                                        e.Graphics.DrawString(content, drawFont, drawBrush, drawPoint, drawFormat);
                                    }
                                    else
                                    {
                                        XPoint += 350.0F;
                            ...
                            // Draw Columns
                            int TempYPoint = int.Parse(YPoint.ToString());
                            tmpTop = e.MarginBounds.Top + TempYPoint-50;
                            i = 0;
                            foreach (DataGridViewColumn GridCol in dgv.Columns)
                            {
                                if (!GridCol.Visible) continue;
